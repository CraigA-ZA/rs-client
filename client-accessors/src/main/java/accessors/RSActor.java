package accessors;

public interface RSActor extends RSEntity {
    public boolean getFalse0();

    public RSIterableNodeDeque getHeadbars();

    public byte getHitmarkCount();

    public int[] getHitmarkCycles();

    public int[] getHitmarkTypes();

    public int[] getHitmarkTypes2();

    public int[] getHitmarkValues();

    public int[] getHitmarkValues2();

    public int getMovementFrame();

    public int getMovementFrameCycle();

    public int getMovementSequence();

    public int getNpcCycle();

    public String getOverheadText();

    public int getOverheadTextCyclesRemaining();

    public int getPathLength();

    public RSMoveSpeed[] getPathTraversed();

    public int[] getPathX();

    public int[] getPathY();

    public int getReadySequence();

    public int getRunSequence();

    public int getSequence();

    public int getSequenceDelay();

    public int getSequenceFrame();

    public int getSequenceFrameCycle();
    public int getSize();

    public int getSpotAnimation();

    public int getSpotAnimationFrame();

    public int getSpotAnimationFrameCycle();

    public int getTargetIndex();
    public int getTurnLeftSequence();

    public int getTurnRightSequence();

    public int getWalkBackSequence();

    public int getWalkLeftSequence();

    public int getWalkRightSequence();

    public int getWalkSequence();

    int getX();

    int getY();

    int getDefaultHeight();

    int getOrientation();
}
