...
class Projectile : IdentityMapper.Class() {
    ...
    class isMoving : OrderMapper.InConstructor.Field(Projectile::class, 0) {
        override val predicate = predicateOf<InstructionWrapper> { it.opcode == Opcodes.PUTFIELD && it.fieldType == BOOLEAN_TYPE }
    }

    @DependsOn(advance::class)
    class x : OrderMapper.InMethod.Field(advance::class, 0) {
        override val predicate = predicateOf<InstructionWrapper> { it.opcode == Opcodes.PUTFIELD && it.fieldType == DOUBLE_TYPE }
    }
    ...
    class speedX : OrderMapper.InMethod.Field(advance::class, 1) {
        override val predicate = predicateOf<InstructionWrapper> { it.opcode == Opcodes.GETFIELD && it.fieldType == DOUBLE_TYPE }
    }
