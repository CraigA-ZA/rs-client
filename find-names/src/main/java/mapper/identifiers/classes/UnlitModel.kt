...
class UnlitModel : IdentityMapper.Class() {
    ...
    class faceColors : UniqueMapper.InMethod.Field(recolor::class) {
        override val predicate = predicateOf<InstructionWrapper> { it.isField && it.fieldType == ShortArray::class.type }
    }
    ...
    class faceLabelsAlpha : OrderMapper.InMethod.Field(light::class, -1) {
        override val predicate = predicateOf<InstructionWrapper> { it.opcode == Opcodes.GETFIELD && it.fieldType == Array<IntArray>::class.type }
    }
    ...
    class faceAlphas : OrderMapper.InMethod.Field(light::class, -1) {
        override val predicate = predicateOf<InstructionWrapper> { it.opcode == Opcodes.GETFIELD && it.fieldType == ByteArray::class.type }
    }
    ...
    class faceNormals : IdentityMapper.InstanceField() {
        override val predicate = predicateOf<FieldWrapper> { it.type == type<FaceNormal>().withDimensions(1) }
    }
    ...
    class resize : OrderMapper.InMethod.Method(LocType.getUnlitModel::class, -2) {
        override val predicate = predicateOf<InstructionWrapper> { it.opcode == Opcodes.INVOKEVIRTUAL && it.methodOwner == type<UnlitModel>() && it.methodType.argumentTypes.size == 3 }
    }
