...
//        override val predicate = predicateOf<Instruction2> { it.opcode == Opcodes.PUTFIELD && it.fieldType == Type.INT_TYPE && it.fieldOwner == type<LocType>() }
//    }
...
//    }
    ...
    class recol_s : OrderMapper.InMethod.Field(getUnlitModel::class, 0) {
        override val predicate = predicateOf<Instruction2> { it.opcode == Opcodes.SALOAD }
...
//    }
    ...
    class loadModels : IdentityMapper.InstanceMethod() {
        override val predicate = predicateOf<Method2> { it.returnType == Type.BOOLEAN_TYPE && it.arguments.isEmpty() && Modifier.isFinal(it.access) }
    }

    class loadModelType : IdentityMapper.InstanceMethod() {
        override val predicate = predicateOf<Method2> { it.returnType == Type.BOOLEAN_TYPE && it.arguments.size == 1 }
    }
}