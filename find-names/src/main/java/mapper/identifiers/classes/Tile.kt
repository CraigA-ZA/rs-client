...
class Tile : IdentityMapper.Class() {
    ...
    class floorDecoration : IdentityMapper.InstanceField() {
        override val predicate = predicateOf<FieldWrapper> { it.type == type<FloorDecoration>() }
    }

    @DependsOn(WallDecoration::class)
    class wallDecoration : IdentityMapper.InstanceField() {
        override val predicate = predicateOf<FieldWrapper> { it.type == type<WallDecoration>() }
    }

    @DependsOn(Wall::class)
    class wall : IdentityMapper.InstanceField() {
        override val predicate = predicateOf<FieldWrapper> { it.type == type<Wall>() }
    }

    @DependsOn(ObjStack::class)
    class objStack : IdentityMapper.InstanceField() {
        override val predicate = predicateOf<FieldWrapper> { it.type == type<ObjStack>() }
    }

    @DependsOn(TileModel::class)
    class model : IdentityMapper.InstanceField() {
        override val predicate = predicateOf<FieldWrapper> { it.type == type<TileModel>() }
    }

    @DependsOn(TilePaint::class)
    class paint : IdentityMapper.InstanceField() {
        override val predicate = predicateOf<FieldWrapper> { it.type == type<TilePaint>() }
    }

    @DependsOn(Scene.newObjStack::class)
    class sceneryCount : UniqueMapper.InMethod.Field(Scene.newObjStack::class) {
        override val predicate = predicateOf<InstructionWrapper> { it.opcode == Opcodes.GETFIELD && it.fieldType == INT_TYPE && it.fieldOwner == type<Tile>() }
    }
...
//    }
    ...
    class linkedBelowTile : IdentityMapper.InstanceField() {
        override val predicate = predicateOf<FieldWrapper> { it.type == type<Tile>() }
    }

    @DependsOn(Scene.setTileMinPlane::class)
    class minPlane : UniqueMapper.InMethod.Field(Scene.setTileMinPlane::class) {
        override val predicate = predicateOf<InstructionWrapper> { it.opcode == PUTFIELD && it.fieldType == INT_TYPE && it.fieldOwner == type<Tile>() }
    }
    ...
    class drawPrimary : OrderMapper.InMethod.Field(Scene.draw::class, 0) {
        override val predicate = predicateOf<InstructionWrapper> { it.opcode == PUTFIELD && it.fieldType == BOOLEAN_TYPE && it.fieldOwner == type<Tile>() }
    }
