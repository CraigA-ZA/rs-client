...
//    }
    ...
    class rotateZ : OrderMapper.InMethod.Method(Projectile.getModel::class, 0) {
        override val predicate = predicateOf<InstructionMapper> { it.isMethod && it.methodOwner == type<Model>() }
    }
...
//    }
    ...
    class rotateY90Ccw : OrderMapper.InMethod.Method(Player.getModel::class, -1) {
        override val predicate = predicateOf<InstructionMapper> { it.opcode == INVOKEVIRTUAL && it.methodOwner == type<Model>() && it.methodType.argumentTypes.size in 0..1 }
    }
...
//    }
    ...
    class isSingleTile : UniqueMapper.InMethod.Field(Player.getModel::class) {
        override val predicate = predicateOf<InstructionMapper> { it.isField && it.fieldOwner == type<Model>() && it.fieldType == BOOLEAN_TYPE }
    }
    ...
    class vertexLabels : OrderMapper.InMethod.Field(transform::class, 0) {
        override val predicate = predicateOf<InstructionMapper> { it.opcode == GETFIELD && it.fieldType == INT_TYPE.withDimensions(2) }
    }
...
//    }
    ...
    class toSharedSequenceModel : UniqueMapper.InMethod.Method(NPCType.getModel::class) {
        override val predicate = predicateOf<InstructionMapper> { it.isMethod && it.methodOwner == type<Model>() && it.methodType.returnType == type<Model>() }
    }

    @DependsOn(transform::class)
    class faceAlphas : UniqueMapper.InMethod.Field(transform::class) {
        override val predicate = predicateOf<InstructionMapper> { it.opcode == GETFIELD && it.fieldType == ByteArray::class.type }
    }
    ...
    class faceColors1 : OrderMapper.InMethod.Field(UnlitModel.light::class, 0) {
        override val predicate = predicateOf<InstructionMapper> { it.opcode == PUTFIELD && it.fieldOwner == type<Model>() && it.fieldType == IntArray::class.type }
    }
    ...
    class faceTextures : OrderMapper.InMethod.Field(UnlitModel.light::class, -1) {
        override val predicate = predicateOf<InstructionMapper> { it.opcode == PUTFIELD && it.fieldOwner == type<Model>() && it.fieldType == ShortArray::class.type }
    }
}