...
class AbstractArchive : IdentityMapper.Class() {
    ...
    class fileNameHashes : OrderMapper.InMethod.Field(decodeIndex::class, -1, 2) {
        override val predicate = predicateOf<Instruction2> { it.opcode == Opcodes.PUTFIELD && it.fieldType == Type.INT_TYPE.withDimensions(2) }
    }
...
//                .and { it.instructions.count { it.opcode == ACONST_NULL } == 5 }
    ...
    class fileIds : OrderMapper.InMethod.Field(decodeIndex::class, 0) {
        override val predicate = predicateOf<Instruction2> { it.opcode == Opcodes.PUTFIELD && it.fieldType == Array<IntArray>::class.type }
    }
}