...
abstract class StringsUniqueMapper(string: String) : UniqueMapper.InClassInitializer.Field(Strings::class) {
    override val predicate = predicateOf<InstructionWrapper> { it.opcode == LDC && it.ldcCst == string }
...
abstract class ArchiveFieldMapper(order: Int) : StaticOrderMapper.Field(order) {
    override val predicate = predicateOf<InstructionWrapper> { it.opcode == INVOKESTATIC && it.methodId == method<Client.newArchive>().id }
            .next { it.opcode == PUTSTATIC && it.fieldType == type<Archive>() }
}

@DependsOn(EvictingDualNodeHashTable::class)
abstract class CachedDefinitionMapper(classMapper: KClass<out Mapper<ClassWrapper>>) : StaticUniqueMapper.Field() {
    override val predicate = predicateOf<InstructionWrapper> { it.opcode == CHECKCAST && it.typeType == context.classes.getValue(classMapper).type }
...
abstract class IndexedSpritesFieldMapper(s: String) : AllUniqueMapper.Field() {
    override val predicate = predicateOf<InstructionWrapper> { it.opcode == LDC && it.ldcCst == s }
...
abstract class AbstractFontStaticIntMapper(index: Int) : OrderMapper.InClassInitializer.Field(AbstractFont::class, index) {
    override val predicate = predicateOf<InstructionWrapper> { it.opcode == PUTSTATIC && it.fieldType == Type.INT_TYPE }
}
...
abstract class PlayerTypeInstance(index: Int) : OrderMapper.InClassInitializer.Field(PlayerType::class, index) {
    override val predicate = predicateOf<InstructionWrapper> { it.opcode == PUTSTATIC && it.fieldType == type<PlayerType>() }
}
...
abstract class WorldMapCacheNameInstance(index: Int) : OrderMapper.InClassInitializer.Field(WorldMapCacheName::class, index) {
    override val predicate = predicateOf<InstructionWrapper> { it.opcode == PUTSTATIC && it.fieldType == type<WorldMapCacheName>() }
}
...
abstract class ByteArrayPoolArray(index: Int) : OrderMapper.InClassInitializer.Field(ByteArrayPool::class, index) {
    override val predicate = predicateOf<InstructionWrapper> { it.opcode == PUTSTATIC && it.fieldType == Type.BYTE_TYPE.withDimensions(2) }
}
...
abstract class LoadRegionPutStatic(type: Type, index: Int) : OrderMapper.InMethod.Field(Client.loadRegions::class, index) {
    override val predicate = predicateOf<InstructionWrapper> { it.opcode == PUTSTATIC && it.fieldType == type }
}
...
abstract class SpriteMaskConstructorField(type: Type, index: Int) : OrderMapper.InConstructor.Field(SpriteMask::class, index) {
    override val predicate = predicateOf<InstructionWrapper> { it.opcode == PUTFIELD && it.fieldType == type }
}
...
abstract class ConstructorPutField(klass: KClass<out Mapper<ClassWrapper>>, index: Int, type: Type) : OrderMapper.InConstructor.Field(klass, index) {
    override val predicate = predicateOf<InstructionWrapper> { it.opcode == PUTFIELD && it.fieldType == type }
}

abstract class MethodPutField(method: KClass<out Mapper<MethodWrapper>>, index: Int, type: Type) : OrderMapper.InMethod.Field(method, index) {
    override val predicate = predicateOf<InstructionWrapper> { it.opcode == PUTFIELD && it.fieldType == type }
}
...
abstract class WorldMapLabelSizeConstant(val index: Int) : OrderMapper.InClassInitializer.Field(WorldMapLabelSize::class, index) {
    override val predicate = predicateOf<InstructionWrapper> { it.opcode == PUTSTATIC && it.fieldType == type<WorldMapLabelSize>() }
}

@DependsOn(TriBool::class)
abstract class TriBoolConst(index: Int) : OrderMapper.InClassInitializer.Field(TriBool::class, index) {
    override val predicate = predicateOf<InstructionWrapper> { it.opcode == PUTSTATIC && it.fieldType == type<TriBool>() }
}
...
abstract class ScriptField(index: Int, type: Type) : OrderMapper.InMethod.Field(Client.loadClientScript::class, index) {
    override val predicate = predicateOf<InstructionWrapper> { it.opcode == PUTFIELD && it.fieldType == type && it.fieldOwner == type<ClientScript>() }
}
...
//    override val predicate = predicateOf<InstructionWrapper> { it.opcode == BIPUSH && it.intOperand == 20 }
//            .nextIn(2) { it.opcode == PUTFIELD && it.fieldType == IntArray::class.type }
//}

@DependsOn(Component.decodeLegacy::class)
abstract class Widget10Array(index: Int) : OrderMapper.InMethod.Field(Component.decodeLegacy::class, index) {
    override val predicate = predicateOf<InstructionWrapper> { it.opcode == NEWARRAY && it.intOperand == 10 }
...
abstract class WidgetListener(index: Int) : OrderMapper.InMethod.Field(Component.decode::class, index) {
    override val predicate = predicateOf<InstructionWrapper> { it.opcode == PUTFIELD && it.fieldType == Array<Any?>::class.type }
}
...
abstract class WidgetListenerTriggers(index: Int) : OrderMapper.InMethod.Field(Component.decode::class, index) {
    override val predicate = predicateOf<InstructionWrapper> { it.isMethod && it.methodId == method<Component.readListenerTriggers>().id }
            .next { it.opcode == PUTFIELD && it.fieldType == IntArray::class.type }
}
...
abstract class SpriteArrayField(archiveField: KClass<out Mapper<FieldWrapper>>) : StaticUniqueMapper.Field() {
    override val predicate = predicateOf<InstructionWrapper> { it.opcode == GETFIELD && context.fields[archiveField]!!.id == it.fieldId }
            .prevWithin(10) { it.opcode == GETSTATIC && it.fieldType == type<Sprite>().withDimensions(1) }
}

@DependsOn(IndexedSprite::class)
abstract class IndexedSpriteArrayField(archiveField: KClass<out Mapper<FieldWrapper>>) : StaticUniqueMapper.Field() {
    override val predicate = predicateOf<InstructionWrapper> { it.opcode == GETFIELD && context.fields[archiveField]!!.id == it.fieldId }
            .prevWithin(10) { it.opcode == GETSTATIC && it.fieldType == type<IndexedSprite>().withDimensions(1) }
}

@DependsOn(Client.runScript::class, ClientScriptEvent::class)
abstract class ScriptEventFieldConst(ldc: Int) : UniqueMapper.InMethod.Field(Client.runScript::class) {
    override val predicate = predicateOf<InstructionWrapper> { it.opcode == LDC && it.ldcCst == ldc }
            .nextWithin(8) { it.opcode == GETFIELD && it.fieldOwner == type<ClientScriptEvent>() }
}
...
abstract class OccluderField(index: Int) : OrderMapper.InMethod.Field(Client.Scene_addOccluder::class, index) {
    override val predicate = predicateOf<InstructionWrapper> { it.opcode == PUTFIELD && it.fieldType == Type.INT_TYPE && it.fieldOwner == type<Occluder>() }
}