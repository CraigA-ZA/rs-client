...
import deobfuscator.deobs.impl.ASM.MultiplierFinderUtilities.invert
import deobfuscator.deobs.impl.ASM.MultiplierFinderUtilities.isInvertible
...
class MultiplierFinder : AbstractDeob() {
    ...
    override fun run() {
        ...
        for (c in Deobfuscator.classMapASM.values) {
            for (m in c.methods) {
        ...
        mapper.writeValue(Constants.MULTIPLIER_JSON.toFile(), multipliers.decoders.toSortedMap())
    }
