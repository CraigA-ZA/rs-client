...
public class MultiplierFinderConverted extends AbstractDeob {
    ...
    class Interpret extends Interpreter<Valu> {
        ...
        @Override
        public Valu newValue(Type type) {
        ...
        private void setField(Valu put, Valu value) {
            ...
            if (value.isLdcInt()) {
                ...
                distribute((FieldInsnNode) put.v.insns.stream().findFirst().orElse(null), (Valu.Two) value);
            }
        }
            ...
            if (ldc != null && other != null) {
                ...
                if (isMultiplier(n) && ldcs.add(ldc)) {
                    ...
                    if (getField == null) {
                        ...
                        multipliers.decEncX.add(new FieldMulAssign(put.name, getField.fieldName(), n));
                    }
                }
                if (value.isMul()) {
                    return;
    ...
    private static class Valu implements Value {
        ...
        public boolean isLdcInt() {
            AbstractInsnNode insn = v.insns.stream().findFirst().orElseThrow(() -> new RuntimeException("No instruction found"));
            return insn != null && insn instanceof LdcInsnNode && (((LdcInsnNode) insn).cst instanceof Integer || ((LdcInsnNode) insn).cst instanceof Long);
        }
        ...
        public Number ldcNum() {
            AbstractInsnNode insn = v.insns.stream().findFirst().orElseThrow(() -> new RuntimeException("No instruction found"));
            return ((LdcInsnNode) insn).cst instanceof Number ? (Number) ((LdcInsnNode) insn).cst : null;
        }

        public String fieldName() {
            AbstractInsnNode insnNode = v.insns.stream().findFirst().orElseThrow(() -> new RuntimeException("No instruction found"));
            if(insnNode instanceof FieldInsnNode) {

                return ((FieldInsnNode) insnNode).owner + "." + ((FieldInsnNode) insnNode).name;
            } else {
                throw new RuntimeException("Shit was not a fieldInsnNode");
            }
        }
