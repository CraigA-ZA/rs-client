...
public class UnusedParamsV2NotMine extends AbstractDeob {
    ...
    public static ArrayList<FoundMethod> dummyParamMethods = new ArrayList();
        ...
        for (ClassNode classNode : Deobfuscator.classMapASM.values()) {
            ...
            for (MethodNode Method : methodList) {
                if (Method.access != Opcodes.ACC_ABSTRACT) {
                    if (Method.name.contains("<") || Method.desc.contains("bouncy"))
                        continue;
                    int paramCount;
